...
(function($){
	...
	var init = function( tb, options){
		...
		t.origWidth = t.w;
		t.minTableWidth = options.minTableWidth;
	...
	var createGrips = function(t){
		...
		var th = t.find(">thead>tr>th,>thead>tr>td").not('.no-resize');	//if table headers are specified in its semantically correct tag, are obtained
		if(!th.length) th = t.find(">tbody>tr:first>th,>tr:first>th,>tbody>tr:first>td, >tr:first>td").not('.no-resize');	 //but headers can also be included in different ways
		t.cg = t.find("col").not('.no-resize'); 						//a table can also contain a colgroup with col elements
		...
		t.cg.each(function(i) {
		  $(this).width($(t.c[i]).width()); //force widths to be in px
		});
	...
	*/
	var syncCols = function(t,i,isOver){
		...
		var w = Math.max( c.w + inc, t.opt.minWidth); //their new width is obtained
		inc = w - c.w;

    c.width( w + PX); //and set
    t.cg.eq(i).width( w + PX);

		if ( t.width() + inc < t.origWidth && t.minTableWidth) { //update last column with any leftover width
		  var lastIndex = t.c.length - 1;
		  var newWidth = t.origWidth - (t.width() + inc) + t.c[lastIndex].width();
		  t.width(t.origWidth);
		  t.c[lastIndex].width( newWidth + PX);
	    t.cg.eq(lastIndex).width( newWidth + PX);
		} else {
		  t.width( t.width() + inc);
		}
	...
	var onGripDrag = function(e){
		...
		var min = i? t.g[i-1].position().left+t.cs+mw : t.c[0].position().left + t.cs+mw;				//min position according to the contiguous cells
		//var min = t.g[i-1].position().left+t.cs+mw    //min position according to the contiguous cells

		x = M.max(min, x)						//apply boundings
	...
	var onGripDragOver = function(e){
		...
		t.find('td').removeClass('show-cell-border');
		t.addClass('col-resized');
	...
	var onGripMouseDown = function(e){
		...
		var headerCol = t.c[g.i], colIndex = 0;

		t.find(">thead>tr>th,>thead>tr>td").each(function(){
		  if ($(this).eq(0)[0] == headerCol.eq(0)[0]) {
		    return false;
		  }
		  colIndex++;
		});

		 t.find('td:nth-child('+(colIndex + 1)+')').addClass('show-cell-border');
    ...
    $.fn.extend({
        colResizable: function(options) {
            var defaults = {
				...
				minTableWidth: true,    //disable table from being dragged below it's original size
